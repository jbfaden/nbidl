...
class OutputDocument implements Document, Element, ChangeListener, ActionListener, Runnable {
    ...
    OutputDocument(OutWriter writer) {
        if (PvwaveController.log) {
            PvwaveController.log ("Creating a Document for " + writer);
        }
        this.writer = writer;
     ...
     */
    public void dispose() {
        if (PvwaveController.log) PvwaveController.log ("Disposing document and backing storage for " + getLines().readLock());
        disposeQuietly();
        writer.dispose();
        writer = null;
    }
    ...
    public void stateChanged(ChangeEvent changeEvent) {
        if (PvwaveController.verbose) PvwaveController.log (changeEvent != null ? "Document got change event from writer" : "Document timer polling");
        if (dlisteners.isEmpty()) {
            if (PvwaveController.verbose) PvwaveController.log ("listeners empty, not firing");
            return;
        }
        if (getLines().checkDirty(true)) {
            if (lastEvent != null && !lastEvent.isConsumed()) {
                if (PvwaveController.verbose) PvwaveController.log ("Last event not consumed, not firing");
                return;
            }
            ...
            if (PvwaveController.verbose) PvwaveController.log ("Document may fire event - last fired getLine=" + lastFiredLine + " getLine count now " + lineCount);
            if (lastFiredLine != lastPostedLine || noPostedLine) {
                lastEvent = new DO(Math.max(0, lastFiredLine), noPostedLine);
//                evts.add (lastEvent);
                Mutex.EVENT.readAccess (new Runnable() {
                    public void run() {
                        if (PvwaveController.verbose) PvwaveController.log("Firing document event on EQ with start index " + lastEvent.start);
                        fireDocumentEvent (lastEvent);
                    }
                });
                lastFiredLine = lastPostedLine;
            } else {
                if (PvwaveController.verbose) PvwaveController.log ("Line count is still " + lineCount + " - not firing");
            }
        } else {
            if (PvwaveController.verbose) PvwaveController.log ("Writer says it is not dirty, firing no change");
        }
        updateTimerState();
    }    
    ...
    private synchronized void updateTimerState() {
        ...
        if (timer == null && getLines().isGrowing()) {
            if (PvwaveController.log) PvwaveController.log("Starting timer");
...
//            logInfo();
        ...
        if (timer != null && timer.getDelay() < 350) {
            ...
            if (PvwaveController.verbose) PvwaveController.log ("Decreased timer interval to " + timer.getDelay());
        }
...
//                synchronized (writer) {
                    consumed = true;
                    if (PvwaveController.verbose) PvwaveController.log ("EVENT CONSUMED: " + start);
