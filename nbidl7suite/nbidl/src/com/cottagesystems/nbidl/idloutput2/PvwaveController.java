 ...
 */
public class PvwaveController { //XXX public only for debug access to logging code

    public static void ensureViewInDefault (final PvwaveIO io, final boolean reuse) {
...
//            KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0));
    ...
    private OutputTab createOutputTab (OutputWindow win, PvwaveIO io, boolean activateContainer, boolean reuse) {
     ...
     */
    private OutputTab createAndInstallView (OutputWindow win, PvwaveIO io) {
...
//            findPreviousAction.setEnabled (out != null && out.getLines().getForwardMatcher() != null);
    ...
    private static File showFileChooser (JComponent owner) {
        File f = null;
        String dlgTtl = NbBundle.getMessage (PvwaveController.class, "TITLE_SAVE_DLG"); //NOI18N
        ...
        if (f != null && f.exists() && !isAqua) { //Aqua's file dialog takes care of this
            String msg = NbBundle.getMessage(PvwaveController.class,
                "FMT_FILE_EXISTS", new Object[] { f.getName() }); //NOI18N
            String title = NbBundle.getMessage(PvwaveController.class,
    ...
    public void notifyInput(OutputWindow win, OutputTab tab, String txt) {
        if (PvwaveController.log) PvwaveController.log ("Notify input on " + tab + " - " + txt);
        PvwaveIO io = tab.getIO();
        if (io != null) {
            PvwaveIO.IOReader in = io.in();
            if (in != null) {
                if (PvwaveController.log) PvwaveController.log ("Sending input to " + in);

                in.pushText (txt + "\n");
				//#56070 - copy input to output, TODO - make it more different color..
            // Don't do local echo    io.getOut().println(txt);
            }
        }
    }
     ...
     */
    public void performCommand(OutputWindow win, OutputTab tab, PvwaveIO io, int command,
        ...
        switch (command) {
            case IOEvent.CMD_SET_PROMPT:
                tab.setPrompt( (String)data );
                break;
...
//                tab.setDocument (new OutputDocument((OutWriter)io.getOut()));
    ...
    private void navigateToFirstErrorLine (OutputTab comp) {
        ...
        if (out != null) {
            ...
            if (PvwaveController.log) PvwaveController.log ("NAV TO FIRST LISTENER LINE: " + line);
            if (line > 0) {
    ...
    private static class ControllerAction extends AbstractAction {
        ...
        ControllerAction (int id, String bundleKey) {
            if (bundleKey != null) {
                String name = NbBundle.getMessage(PvwaveController.class, bundleKey);
        ...
        private static KeyStroke getAcceleratorFor (String name) {
            ...
            return Utilities.stringToKey(NbBundle.getMessage(PvwaveController.class, key));
        }
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            PvwaveController cont = win.getController();
            if (cont != null) {
    ...
    static class ControllerOutputEvent extends OutputEvent {
        private int line;
        ControllerOutputEvent (PvwaveIO io, int line) {
            super (io);
            this.line = line;
        }
        ...
        public String getLine() {
            PvwaveIO io = (PvwaveIO) getSource();
            OutWriter out = io.out();
            try {
                if (out != null) {
    ...
    private static OutputStream getLogStream() {
        if (logStream == null) {
            ...
            synchronized (PvwaveController.class) {
                try {
    ...
    public void inputEof(OutputTab tab) {
        if (PvwaveController.log) PvwaveController.log ("Input EOF");
        PvwaveIO io = tab.getIO();
        PvwaveIO.IOReader in = io.in();
        if (in != null) {
            in.eof();
