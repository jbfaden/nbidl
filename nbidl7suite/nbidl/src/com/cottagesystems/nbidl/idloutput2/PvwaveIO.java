 ...
 * @author  Tim Boudreau
 */
public class PvwaveIO implements InputOutput {
     ...
     */
    PvwaveIO(String name, Action[] toolbarActions) {
        this(name);
        this.actions = toolbarActions;
    }
    
    /** Package private constructor for unit tests */
    PvwaveIO (String name) {
        this.name = name;
    }
    
    public void closeInputOutput() {
        if (PvwaveController.log) PvwaveController.log("CLOSE INPUT OUTPUT CALLED FOR " + this);
        if (out != null) {
            if (PvwaveController.log) PvwaveController.log (" - Its output is non null, calling close() on " + out);
            out.close();
        }
    ...
    public void setPrompt( String prompt ) {
      post( this, IOEvent.CMD_SET_PROMPT, prompt );
    }
    
    void dispose() {
        if (PvwaveController.log) PvwaveController.log (this + ": IO " + getName() + " is being disposed");
        if (out != null) {
            if (PvwaveController.log) PvwaveController.log (this + ": Still has an OutWriter.  Disposing it");
            out().dispose();
            out = null;
            if (in != null) {
                in.eof();
    ...
    public void select() {
        if (PvwaveController.log) PvwaveController.log (this + ": select");
    ...
    public void setInputVisible(boolean value) {
        if (PvwaveController.log) PvwaveController.log(PvwaveIO.this + ": SetInputVisible");
    ...
    public void setStreamClosed(boolean value) {
        if (PvwaveController.log) PvwaveController.log ("setStreamClosed on " + this + " to " + value);
    ...
    public void reset() {
        if (PvwaveController.log) PvwaveController.log (this + ": reset");
        closed = null;
    ...
    private static void post (PvwaveIO io, int command, boolean val) {
        IOEvent evt = new IOEvent (io, command, val);
        post (evt);
    }

    private static void post (PvwaveIO io, int command, Object data) {
    ...
    static void post (IOEvent evt) {
        if (SwingUtilities.isEventDispatchThread()) {
            if (PvwaveController.log) PvwaveController.log ("Synchronously dispatching " + evt + " from call on EQ");
            evt.dispatch();
        } else {
            if (PvwaveController.log) PvwaveController.log ("Asynchronously posting " + evt + " to EQ");
    ...
    class IOReader extends Reader {
        ...
        public void pushText (String txt) {
            if (PvwaveController.log) PvwaveController.log (PvwaveIO.this + ": Input text: " + txt);
            synchronized (lock) {
                buffer().append (txt);
                lock.notifyAll();
            }
        ...
        private void checkPristine() throws IOException {
            ...
            if (pristine) {
                PvwaveIO.this.setInputVisible(true);
                pristine = false;
            }
        }
       
        public int read(char cbuf[], int off, int len) throws IOException {
             if (PvwaveController.log) PvwaveController.log  (PvwaveIO.this + ":Input read: " + off + " len " + len);
            checkPristine();
            synchronized (lock) {
        ...
        public int read() throws IOException {
            if (PvwaveController.log) PvwaveController.log (PvwaveIO.this + ": Input read one char");
            checkPristine();
            synchronized (lock) {
                while (!inputClosed && buffer().length() == 0) {
                    try {
        ...
        public long skip(long n) throws IOException {
            if (PvwaveController.log) PvwaveController.log (PvwaveIO.this + ": Input skip " + n);
            checkPristine();
            synchronized (lock) {
                while (!inputClosed && buffer().length() == 0) {
                    try {
        ...
        public void close() throws IOException {
            if (PvwaveController.log) PvwaveController.log (PvwaveIO.this + ": Input close");
