...
/**
 * This provides toggle breakpoint, etc.
 * @author  Honza
 */
public class PvwaveBreakpointActionProvider extends ActionsProviderSupport
    ...
    public PvwaveBreakpointActionProvider() {
    ...
    public void doAction(Object action) {
        ...
        if (line == null) return ;
        if (!isPvwaveFile((FileObject) line.getLookup().lookup(FileObject.class))) {
            return ;
        }
        ...
        for (i = 0; i < k; i++)
            if ( breakpoints [i] instanceof PvwaveBreakpoint &&
                ((PvwaveBreakpoint) breakpoints [i]).getLine().getLineObject().equals(line)
                ) {
    ...
    private static boolean isPvwaveFile(FileObject fo) {
        if (fo == null) {
            return false;
        } else {
            return fo.getMIMEType().equals("text/x-application-pvwave");
        }
    }
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        ...
        if (line == null) {
            ...
            enabled = isPvwaveFile((FileObject) line.getLookup().lookup(FileObject.class));
        }
